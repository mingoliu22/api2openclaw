...
func (s *PostgreSQLAPIKeyStore) List(ctx context.Context, filter *APIKeyFilter) ([]*APIKey, error) {
	query := `SELECT id, label, key_hash, key_prefix, model_alias, expires_at, status, note, created_at, revoked_at
	...
	for rows.Next() {
		k := &APIKey{}
		err := rows.Scan(&k.ID, &k.Label, &k.KeyHash, &k.KeyPrefix, &k.ModelAlias, &k.ExpiresAt, &k.Status, &k.Note, &k.CreatedAt, &k.RevokedAt)
		if err != nil {
...
func (s *PostgreSQLAPIKeyStore) GetByID(ctx context.Context, id string) (*APIKey, error) {
	...
	err := s.db.QueryRowContext(ctx, query, id).Scan(
		...
		&k.Status, &k.Note, &k.CreatedAt, &k.RevokedAt,
	)
	if err != nil {
...
func (s *PostgreSQLAPIKeyStore) Create(ctx context.Context, key *APIKey) error {
	...
	query := `INSERT INTO api_keys (id, label, key_hash, key_prefix, model_alias, expires_at, status, note, created_at)
	          VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`

	_, err := s.db.ExecContext(ctx, query,
		key.ID, key.Label, key.KeyHash, key.KeyPrefix, key.ModelAlias,
		key.ExpiresAt, key.Status, key.Note, key.CreatedAt,
	)
	if err != nil {
